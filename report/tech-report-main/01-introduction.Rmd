...
In this version, we update the report to include 2025 data.
...
The report covers `r no_species` fish species that are either of commercial, recreational, conservation, or First Nations interest, or are regularly caught in our research surveys.
...
* We rebuilt all figure pages with data up to 2025 using the methods described in @anderson2019synopsis, @anderson2020fisheries, @synopsis2021data, @synopsis2022data, @synopsis2023data, and @synopsis2024data.
* We updated the recent references at the top of each species page.
* We added pages for Pacific Herring, American Shad, Pink Salmon, Chum Salmon, Coho Salmon, Sockeye Salmon, and Chinook Salmon.
* We added a table documenting which length measurement type is displayed for each species (Table \@ref(tab:survey-length-types)).
* We raised the limit on the average coefficient of variation above which we do not display the geostatistical indices of abundance from 1.0 to 1.25.
* We added snout-to-anal-fin length measurements for all grenadiers.
* We raised the upper y-axis limit for the "Growth" panels to encompass all the observations, rather than cutting it off at 20% above the fitted growth curve.
* We corrected the algorithm that is designed to simplify the fitted spatiotemporal index standardization model if more complex versions fail to converge. The new version makes use of the new option `collapse_spatial_variance` in the sdmTMB R package [@anderson2022sdmTMB]. With this option enabled, if the standard deviation of a given random effect component is estimated to be below 0.01, the model is refit with the component turned off (fixed to zero).
* We used a new function in the [gfdata](https://github.com/pbs-assess/gfdata) R package to extract the survey set data: `get_all_survey_sets()`. The output was tested to ensure an exact match to the data as extracted via `get_survey_sets()`. As part of this transition, we included a small number of sets in our geostatistical index modelling that lacked a stratum assignment (`grouping_code`) but were otherwise valid sets. This added 2 sets for HBLL INS N in 2007, 1 set for HBLL INS S in 2005 and 1 set in 2009, 1 set for SYN QCS in 2015, and 1 set for SYN WCVI in 2008. We kept these sets excluded for the calculation of the design-based indices.
* We transitioned the calculation of the design-based indices to the [gfdata](https://github.com/pbs-assess/gfdata) R package. These indices are now cached in a data frame at `gfdata::design_indexes` and are generated from a script at `data-raw/design-indexes.R`. This transition was made because of a decision to no longer store the design-based indices in the DFO groundfish database `GFBio`. The code for calculating the bootstrapped indices was verified to generate identical indices. The index calculation and bootstrapping is done within a new function `gfdata::get_design_index()`.
* We added text describing triggers (or "exceptional circumstances") for potential early reassessment for those stocks where a quantitative trigger was stated in the latest Science Advisory Report or Science Response. In this report, we added such text for Dover Sole and Silvergray Rockfish and evaluate each stock against the stated triggers.
* We added yellow highlighted boxes behind any species-specific "notes". This change was based on feedback that the previous notes could be easily missed.
* We added a web version of the report at <https://gfsynopsis.ca>. The web version should render appropriately on desktop screens as well as mobile devices. We intend to continue publishing an annual archive as a Technical Report. We encourage users to use whichever version is most convenient to them, but to cite this latest Technical Report as well as one or both of @anderson2019synopsis and @anderson2020fisheries. We may update the web version in between the annual Technical Reports.
