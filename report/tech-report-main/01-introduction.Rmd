...
* We transitioned the calculation of the design based indices to the [gfdata](https://github.com/pbs-assess/gfdata) R package. These indices are now cached in a data frame at `gfdata::design_indexes` and are generated from a script at `data-raw/design-indexes.R`. This transition was made because of a decision to no longer store the design-based indices in GFBio. The code for calculating the bootstrapped indices was verified to generate identical indices. The index calculation and bootstrapping is done within a new function `gfdata::get_design_index()`.
* We added a web version of the report at <https://gfsynopsis.ca>. We encourage users to use the web version if convenient but to cite this latest Technical Report as well as one or both of @anderson2019synopsis and @anderson2020fisheries.
