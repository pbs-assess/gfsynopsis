...
Fisheries and Oceans, Canada (DFO) conducts a number of fishery-independent
...
(*Hippoglossus stenolepis*). In 2003, the IPHC provided the opportunity to
...
\begin{equation}
  B_t = \sum_{i = 1}^k C_{yt,i} A_i
\end{equation}

where $C_{t,i}$ represents the mean CPUE in parts (fish) per
km\textsuperscript{2} for the species in year $yt$ and stratum $i$, $A_i$
...
of $B_t$, $B_t^{\mathrm{rep}}$, and calculated 95% quantile-based confidence
intervals on $B_t^{\mathrm{rep}}$.
...
In recent years there has been a movement toward model-based standardization of
survey biomass index trends (e.g. REFS). This approach has a theoretical
advantage over a design-based approach for a number of reasons ... TODO.

TODO: add a few sentences here about the general reasons why we might prefer
a spatial model over a design based model

We include model-based index trends for the synoptic trial surveys as a point
of comparison so the reader can gauge when the two approaches may differ. Large
differences are likely a result of the random positioning of the survey sets
for a given year ending up in particularly good or poor habitat for a given
species. Authors of groundfish stock assessments may want to consider
model-based index standardization on a case-by-case basis.
...
with covariance matrice $\bm{\Sigma}_{\epsilon,t}$:
...
the spatial model described in Appendix \@ref(sec:spatial-modeling) to make the
...
\@ref(fig:sdmTMB-spt-plot-all-effects-sqrt)). We can also look at the
...
\begin{equation}
b_t = \sum_{j = 1}^{n_j}
  w_j \exp \left( \bm{X}_{j,t} \bm{\beta} + \omega_j + \epsilon_{j,t} \right),
\end{equation}
...
Similar to the findings of @thorson2015, we found that the model-based biomass
index trends often had lower CVs than the design-based index trends and often
helped stabilize biomass estimates for outlying years compared to the
...
with or without depth covariates was in the models with depth covariance having
...
We chose to not include depth and depth squared as predictors in the main index
illustrated throughout this report. Whereas in nearly all cases including depth
covariates generated a similar index to excluding them, in a few cases
including them generated what appeared to be unrealistic deviations in biomass
when a combination of the quadratic shape of the relationship between depth and
biomass combined to generate exceedingly high over low estimates of biomass on
the border of the survey polygons. This remains a topic of research and will be
