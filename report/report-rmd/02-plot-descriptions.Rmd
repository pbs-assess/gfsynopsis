...
p3 <- g$survey_spatial_iphc +
  ...
  ggplot2::scale_fill_viridis_c(trans = trans, option = "C", breaks = c(0.2, 0.4, 0.7)) +
...
catch-per-unit-effort maps for Pacific Cod (note this figure is **not** Petrale Sole). 
...
```{r length-weight-vb, warning=FALSE, fig.asp=0.4, fig.cap=paste0("Example length-age and length-weight model fits and plots for ", sp, ". The length-age growth curve is a von Bertalanffy model. Female model fits are indicated as solid black lines and male model fits are indicated as dashed grey lines. Text on the panels shows the parameter estimates and light dots represent individual fish that the models are fit to. These figures include all commercial and survey samples. See Appendices \\ref{sec:length-age-models} and \\ref{sec:length-weight-models} for details on the models.")}
