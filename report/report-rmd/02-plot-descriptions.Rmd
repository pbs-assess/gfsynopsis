...
spatial random effects (Appendix \@ref(app:spatial-modeling)). The colour scale
...
scale ('Viridis plasma') is perceptually uniform, robust to colourblindness,
...
(ref:catches-cap) Example plots of commercial fishery catches plot for `r sp`.
Catch from various gear types is indicated by colour shading. Catch is
calculated as the weight of landings plus discards aggregated by year; however,
longline discards are considered less reliable prior to 100% observer coverage
in 2006 and are often reported as discard counts without a reported weight and
are not therefore not included. Years before 1996 and 2006 are shaded grey to
indicate that they are considered less reliable: an at-sea observer program was
...
catch-per-unit-effort maps for `r sp`. Lighter shading indicates higher levels
...
colour scale ('Viridis') is perceptually uniform, robust to colourblindness,
...
shown from 2013 onwards when the trawl footprint was frozen. For hook and line,
...
```{r cpue-maps, fig.asp=1.47, fig.width=4.1, out.width="3.4in", fig.cap="(ref:cpue-maps-cap)"}
gridExtra::grid.arrange(
