...
pkgs <- sort(c("broom", "gfplot", "ggplot2", "dplyr", "rstan", "knitr", "glmmTMB",
  "TMB", "broom.mixed", "purrr", "xtable", "kableExtra", "cowplot", 
...
The data extraction and many of the plots were made with
the gfplot package Git SHA (Secure Hash Algorithm) version ``r sha_gfplot``.
The spatial and spatiotemporal models were fit with the sdmTMB package Git SHA version ``r sha_sdmTMB``.
The plots were assembled on figure pages and this text was written with the package gfsynopsis Git SHA version ``r sha_gfsynopsis``.
...
\vspace{5mm}
Versions used to render the final report will be placed on an archival server.

\clearpage

The figure pages can be built while on the PBS network by (1) installing the above packages, (2) cloning the gfsynopsis repository, and (3) running, with the `gfsynopsis` folder as the working directory:
