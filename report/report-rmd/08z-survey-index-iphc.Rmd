# IPHC SURVEY INDEX {#sec:iphc-survey-index}


<!-- Copying in from AppD-SurveysHookYYR15rev2.tex, simplifying down first in
 simplify.tex, then incrementally copying in what I want to here and converting to .Rmd-->
...
TODO Verify Table 4 values for an example species (e.g. YYR).
...
Each year, some stations may be declared unusable by the IPHC and so are
excluded from our analyses. Stations that were declared as 

...TODO-check-name 
'unusable for spatiotemporal analysis' are more common and are included.

TODO include code tables (in my hooks .Snw)
...
paired t-test REFcrawley on the scaled annual means for the overlapping years, to test the null
...
TODO (whole section) Hook competition occurs ...... REF Webster and Clark...

TODO  decide what to do The Yelloweye
...
From page 6 of my notes..... define

species $s$ (not $i$, which was from following Webster etc.'s notation)
...
So the competition adjustment factor 
\begin{equation}
\frac{-\ln P_{it}}{1 - P_{it}}
\end{equation}
scales up the
observed number of each species caught to give the expected number (accounting
for hook competition), depending on the proportion $P_{it}$ of observed hooks
that are returned still with bait on them.

Note that, by definition, $0 \leq P_{it} \leq 1$, and so $\ln P_{it} \leq 0$. As
$P_{it} \rightarrow 1$ then by L'H\mbox{\^o}pital's Rule REF
\begin{align}
\lim_{P_{it} \rightarrow 1} \frac{-\ln P_{it}}{1 - P_{it}} & = 
...
```{r iphc-hook-with-bait-counts}
bait_set_counts <- readRDS(paste0(file.path(dc, "iphc/hook-with-bait.rds")))$set_counts

```

As $P_{it} \rightarrow 0$, the competition adjustment factor $\rightarrow
\infty$, which blows up the expected number $N_{its}^{(0)}$. In practice this
...
For a set with 800 hooks, the smallest possible positive value is $P_{it} =
1/800$, for which the adjustment factor is 6.69.
...
```{r iphc-hook-with-bait, width=5, fig.cap="Histograms of number of hooks returned with baits in each set, for each year in which all hooks were enumerated. TODO Be better to have proportion - do if time."}
...
```{r iphc-hook-with-bait-twenty, width=5, fig.cap="Histograms of number of hooks out of the first 20 of each skate that were returned with bait, for each year (except 1995 and 1996 for which this cannot be calculated). TODO Be better to have proportion - do if time."}
ggplot(data=bait_set_counts,
       aes(N_it20)) +
    xlab("Number of hooks returned each year") +
    ylab("Frequency") +    
