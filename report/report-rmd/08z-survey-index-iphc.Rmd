...
Define $H_{it}^{(\mathrm{P})}$ to be the proportion of observed chum-bait hooks
...
\begin{equation}
H_{it}^{(\mathrm{P})} = 1 - \frac{\sum_s N_{its}}{H_{it}}
\end{equation}
such that 
\begin{equation}
\sum_s N_{its} = {H_{it}} \left(1 -  H_{it}^{(\mathrm{P})} \right).
\label{eq:sumsNits}
\end{equation}
...
\begin{align}
{{H_{it}}} \left(1 -  H_{it}^{(\mathrm{P})} \right) & = Z_{it} H_{it}
  \frac{\left(1 - \mathrm{e}^{- Z_{it}} \right)}{Z_{it}} \\
1 -  H_{it}^{(\mathrm{P})} & = 
  1 - \mathrm{e}^{- Z_{it}} \\
Z_{it} & = - \ln H_{it}^{(\mathrm{P})}.
\end{align}
