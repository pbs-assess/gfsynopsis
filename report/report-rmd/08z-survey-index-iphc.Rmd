...
```{r iphc-ser-plots, fig.nrow=2, fig.cap=c("Catch rate index (number of individual Yelloweye Rockfish caught per skate) for Series A from the Yelloweye Rockfish assessment (Yamanaka et al., 2018); updated automated figures will be produced for our final report. For a given year, the catch rate for each set is calculated from (\\ref{eq:catchPerSet}). These catch rates were then resampled for 10,000 bootstrap values, from which a bootstrapped mean (open circles) and 95% bias-corrected and adjusted confidence intervals (bars) were calculated. Small black closed circles are sample means (not bootstrapped), and essentially equal the bootstrapped means.", "Series B from the Yelloweye Rockfish assessment."), out.width="3.5in", fig.pos="tb"}
knitr::include_graphics(c("figure/bcaKeepSerA.png", "figure/bcaKeepSerB.png"))
```
