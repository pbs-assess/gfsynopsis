...
\infty$, which blows up the expected number $N_{its}^{(0)}$. TODO - in practice
this happens **CHECK HOW OFTEN, IF EVER**. 

For a set with 800 hooks, the smallest possible positive value is $P_{it} = 1/800$, for
which the adjustment factor is 6.69.
...
```{r iphc-hook-with-bait, fig.cap="Histograms of number of hooks returned with baits in each set, for each year. TODO xlab"}
bait_set_counts <- readRDS(paste0(file.path(dc, "iphc/hook-with-bait.rds")))$set_counts
ggplot(data=bait_set_counts, aes(N_it), xName="P_it") + geom_histogram() + facet_wrap(. ~ year, ncol=4)
