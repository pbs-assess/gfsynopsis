...
REF{yocld08}. An effective skate of one represents a skate of 100 circle
hooks with 18-foot spacing REF{yocld08}.
...
per effective skate.
The catch rates within
a year are then bootstrapped, to give bootstrapped means, bias-corrected and adjusted
...
Columbia waters.   TODO - check those results and mention somewhere.
...
Not needed? This work demonstrates that a consistent index can be constructed despite the
...
20 consecutive hooks (e.g.&nbsp;REF dvr02, dvr03).
...
Sections&nbsp;\ref{sec:compareAD} and&nbsp;\ref{sec:compareBC} we how we determine, for
each species, 
...
TODO - The absolute catch rate index for Series&nbsp;AB could therefore be justified as
being an input to an assessment model, and be considered to be an index for the
whole coast of British Columbia.

TODO Caveat at end? For some species that are only occasionally caught, or were
not identified in the earleir years) sampled the
longest time series may be Series A (if no data are available for 1995 and
1996), Series D (if .....). 

TODO Verify Table values for an example species (e.g. YYR).

## SPATIAL LOCATIONS OF STATIONS

TODO: In the early years
...
excluded from our analyses. Stations that were declared as TODO-check-name 
...
2012, a bait experiment was conducted TODO REF hsdgr13. At each station three
...
Adapting equations on page 3 of REF yocld08, define:
...
hypothesis that there is no difference between the scaled annual means.
If the resulting
$p$-value is $\geq 0.05$ then we cannot reject the null hypothesis. Then we join
...
If the resulting $p<0.05$ then we cannot join the series, and so we stick with
...
species TODO give command. Do one to check years in table. 
...
### CONSTRUCTING SERIES&nbsp;D (20 HOOKS, COASTWIDE) AND COMPARING IT WITH SERIES&nbsp;A (20 HOOKS, NORTH OF VANCOUVER ISLAND
...
a paired t-test on the scaled annual bootstrapped means for the overlapping years.
Again, if $p\geq 0.05$ then we consider the two series comparable.
...
coast. If $p<0.05$ then this is not the case.
...
### CONSTRUCTING SERIES&nbsp;C (ALL HOOKS, COASTWIDE) AND COMPARING IT WITH SERIES&nbsp;B (ALL HOOKS, NORTH OF VANCOUVER ISLAND
...
years and conduct
a paired t-test on the scaled annual bootstrapped means for the overlapping years,
and if $p\geq 0.05$ then we consider the two series comparable.
