...
**GET FIGS IN THEN MOVE THEM**


<!-- Hard-wired ref here but fig and caption will get updated anyway for Res -->
<!-- Doc. No time to create figure for working paper. -->

```{r iphc-ser-plots, fig.cap="Catch rate index (number of individual Yelloweye Rockfish caught per skate) for (a) Series A and (b) Series B from the Yelloweye Rockfish assessment (Yamanaka et al., 2018); updated automated figures will be produced for our final report. For a given year, the catch rate for each set was calculated from (G.2) or (G.5). These catch rates were then resampled for 10,000 bootstrap values, from which a bootstrapped mean (open circles) and 95\\% bias-corrected and adjusted confidence intervals (bars) were calculated. Small black closed circles are sample means (not bootstrapped), and essentially equal the bootstrapped means.", out.width="3.5in", fig.pos="tb"}
knitr::include_graphics("figure/temp2/notscaled.png")
```



```{r iphc-ser-plots-2, fig.cap="(a) Each of the two catch rate series from Figure \\ref{fig:iphc-ser-plots} is divided by the geometric mean of its bootstrapped annual means (with the geometric mean based on the overlapping years only). (b) The catch rate index Series~AB, which extends the original Series~A by incorporating the suitably scaled 1995 and 1996 values from Series~B (see text).",  out.width="3.5in", fig.pos="tb"}
knitr::include_graphics("figure/temp2/rescaled.png")
```
...
competition and unbaited hooks*, unpublished manuscript). This attempted to
