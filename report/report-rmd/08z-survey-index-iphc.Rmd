...
Define $H_{it}^{(\mathrm{B})}$ to be the proportion of observed chum-bait hooks
...
\begin{equation}
H_{it}^{(\mathrm{B})} = 1 - \frac{\sum_s N_{its}}{H_{it}}
\end{equation}
such that 
\begin{equation}
\sum_s N_{its} = {H_{it}} \left(1 -  H_{it}^{(\mathrm{B})} \right).
\label{eq:sumsNits}
\end{equation}
...
\begin{align}
{{H_{it}}} \left(1 -  H_{it}^{(\mathrm{B})} \right) & = Z_{it} H_{it}
  \frac{\left(1 - \mathrm{e}^{- Z_{it}} \right)}{Z_{it}} \\
1 -  H_{it}^{(\mathrm{B})} & = 
  1 - \mathrm{e}^{- Z_{it}} \\
Z_{it} & = - \ln H_{it}^{(\mathrm{B})}.
\end{align}

....Sub 19 and 20 into 14, to give the competition adjustment factor...... do in morning....

\begin{equation}
N_{its}^{(0)} = \frac{- \ln H_{it}^{(\mathrm{B})} N_{its}}{1 - H_{it}^{(\mathrm{B})}},
\label{eq:Nits0rearrange}
\end{equation}
