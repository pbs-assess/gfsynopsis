...
1. Appendix \@ref(sec:age-precision) shows ageing precision plots.

1. Appendix \@ref(sec:depth-curves) shows the predict relationships between
   depth and survey biomass density for the synoptic trawl surveys.

1. Appendix \@ref(sec:data) provides details on the data extraction from the
   relational databases that hold the raw data.

1. Appendix \@ref(sec:cpue-models) provides details on the catch per unit
   effort (CPUE) model that underlies the CPUE visualizations.

1. Appendix \@ref(sec:spatial-modeling) provides details on the spatial
   modeling behind the spatial survey biomass visualizations.

1. Appendix \@ref(sec:survey-trend-models) provides details on the survey
   biomass index modeling (except for the IPHC survey).

1. Appendix \@ref(sec:iphc-survey-index) provides details on the modelling for
   the IPHC survey time series.

1. Appendix \@ref(sec:growth-and-maturity) provides details on modeling
   biological parameters.

1. Appendix \@ref(sec:reproducibility) discusses the computational environment
   that enables reproducibility from data extraction, data manipulation, model
   fitting and visualization to the generation of this report.
...
We also note that the table of contents and references are clickable hyperlinks
