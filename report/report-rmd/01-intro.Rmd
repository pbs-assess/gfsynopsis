...
(PBS) in Nanaimo, British Columbia manages vast quantities of such data on
groundfish species in British Columbia (BC). However, there is not the capacity
...
the report annually or biennially (excluding any unchanged methods) possibly as
...
across all stocks to provide information for discussion of assessment
priorities stocks; (2) generate standardized datasets, biological model fits,
...
(3) increase data transparency between Fisheries and Oceans Canada, the fishing
...
<!--
TODO: Rob: I think somewhere a short paragraph should outline the direction for
meeting this goal
(a) the outputs could help suggest stock where data in-hand could be used to
parameterize operating models versus priors from other sources and assumptions.
(b) that the R functions can port the data to something like DLMtool for rapid
prototyping of at least a basic operating model, and (c) similarly something
like DLMtool can indicate what management procedures might be available for
discussion with Fisheries Management and resource users as well as do the value
of information exercise to see if juice worth the squeeze on data not in-hand.
-->
...
each species (Section \@ref(sec:synopsis-plots). The report covers 112
groundfish species that are either of commercial, recreational, or First
...
layout pages starts with the species common name, the species scientific name,
...
```{r management-map, fig.cap="Map of Pacific Marine Fisheries Commission (PMFC) areas 5AB (Queen Charlotte Sound), 5CD (Hecate Strait), and 3CD (West Coast Vancouver Island). These are close, but not identical, to similarly named Groundfish Management Unit areas.", out.width="3.3in", fig.pos="tb"}
knitr::include_graphics("figure/3CD5ABCDE.png", dpi = NA)
```
...
1. Appendix \@ref(app:reproducibility) describe the computational environment
...
1. Each species is displayed with the same layout to facilitate finding a type
   of data, comparing stocks, and identifying missing data via empty plots.
...
1. The outputs in this report are not a substitute for stock assessment. For
   example, although relative biomass indices from surveys indicate the biomass
   trend for a species in an area, such information is best combined with other
   information such as removals by commercial catches and information on the
   age- or length-composition of the stock to make conclusions about the status
   of a stock. 
...
1. Do not use the outputs as an input to marine spatial planning. TODO

1. Commercial CPUE data should not be considered to be proportional to stock
   abundance. Commercial CPUE time series, even when standardized for
   consistent time of year, depth, latitude, and vessel need to be considered
   even more carefully. There are a multitude of reasons why commercial CPUE
   trends may not represent underlying trends in abundance [e.g., @harley2001].
   Nonetheless, we think there is value in transparently displaying the
   available data for all species. 

1. The catch history reported here reflects recorded data and may not represent
   actual catches.  Uncertainty means that alternative hypotheses for catch
   history are possible. The commercial catch presented here won't necessarily
   match reconstructed time series in stock assessments. Historic catch
   reconstructions require careful species-specific consideration and analysis.
   Furthermore, fluctuations in commercial catch, for example recent declines
   in species catch for Bocaccio and Yelloweye Rockfish, do not necessarily
   reflect declines in stock abundance and may be due to other factors
   including implementation of management measures. Reported discard weights
   are considered less reliable prior to 100% observer coverage of the bottom
   trawl fishery in 1996, and prior to fisheries integration in 2006 for the
   trap, hook and line, midwater trawl and Strait of Georgia (SoG) bottom trawl
   fisheries. The discards in the catch plot therefore only include bottom
   trawl discard weights from 1996 to present and trap, hook and line, midwater
   trawl and SoG bottom trawl discard weights from 2006 to present.

1. Index trends do not resolve population scale and the outputs in this report
   do not resolve conflicts and trends drawn from different sources for the
   same species. TODO

<!-- TODO: make sure the number of species is constant throughout -->

1. An overall caveat of this work is that it is not feasible for us to
   individually assess the results for all 113 species in a detailed manner. To
   use the results for a particular species in future assessments, or to make
   other inferences, we recommend that users carefully examine the data and
   model results. Due to the necessary automation required to construct this
   report, not all species-specific special cases may have been fully
   considered.

<!-- TODO: add "This point actually, should be stated rather firmly in the
"Caveats" section.  The idea discussed in meeting that the outputs should
reflect the question at hand.  Since this document is to "window shop the data"
to use D. Ricard's analogy, the plots are necessarily a compromise relative to
modifications for a specific purpose." -->
...
other groundfish analyses (Figure \@ref(fig:gfplot-web)). Also mention
gfsynopsis and refer to the relevant section at the back. TODO
