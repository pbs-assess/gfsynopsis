...
For the IPHC survey, the data were updated to include 2019, 2020, and 2021
(Table \@ref(tab:IPHCdata)). The data are available at different resolutions
and spatial scales due to the survey design.
<!-- TODO: SA: what does this mean? -->
The methods (and some of the data) have been moved from the gfplot package to
a new R package, gfiphc [@edwards2022gfiphc].
...
1. In 2018 there were 131 new expansion stations that had not been previously
   surveyed. These new stations, plus six further new ones in 2021, are
   excluded from the time-series calculations; they were originally
   inadvertently included for 2018 in the calculations and maps in
   @anderson2019synopsis. TODO: check if include in new maps or not. Best not
   to as some may be outside of current gfsynopsis plotting region.
...
3. The bootstrapping methods use 10000 bootstrap samples instead of 1000, as
   it made a difference for some species.
...
5. Also for 2021, there were six new stations that had never been surveyed
   before. These are declared as non-standard for constructing the various
...
Further technical details are given in @edwards2022gfiphc, which also
includes details for further analyzing data for individual species, groups of
