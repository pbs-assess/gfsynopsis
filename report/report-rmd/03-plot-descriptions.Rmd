...
```{r load-example-figs}
g <- readRDS(here::here("report/report-rmd/ggplot-objects/petrale-sole.rds"))
sp <- "Petrale Sole"
```
...
a spatial model that includes depth and depth squared as predictors as well
as spatial random effects (Appendix E in @anderson2019synopsis). The colour scale
...
an at-sea observer program was implemented for bottom and midwater trawl fleets 
in outside waters in 1996 and an at-sea observer program was implemented for
non-trawl sectors in 2006. Management areas, as indicated in the top left 
corner of each panel, are shown in Figure \@ref(fig:management-map).
...
shown from 2013 onwards after the trawl footprint was frozen. Trawl data from 2007--2012
...
g_alt <- list()
g_alt$cpue_spatial <- readRDS(here::here("report/report-rmd/ggplot-objects/pacific-cod-cpue-spatial.rds"))
g_alt$cpue_spatial_ll <- readRDS(here::here("report/report-rmd/ggplot-objects/pacific-cod-cpue-spatial-ll.rds"))
...
exist in Appendix A in @anderson2019synopsis.
