...
for groundfish [@thorson2015]. Our specific model is implemented with INLA
[@rue2009; @lindgren2015] and TMB [@kristensen2016] via R [@r2018], which
enables rapid model fitting for these large spatial data sets.

At a high level, these models predict relative biomass in space as a continuous
process with a quadratic effect for bottom depth, spatial random effects that
represent an amalgamation of spatial processes not explicitly included in the
model, and an observation error component. After fitting the model to survey
sets from trawl or longline surveys, we then project the model predictions on
to a fine-scale 2 $\times$ 2 km grid in a UTM 9 projection to derive estimates
of biomass throughout the survey domain.

Similarly to the commercial catch per unit effort standardization models
(Section (\@ref(sec:cpue-models))), these models can be represented as Tweedie
...
parameter. The vector $\bm{X_s}$ represents a vector of predictors (here
a 1 for an intercept, depth, and depth squared) and $\bm{\beta}$ represents
...
function, which defines the rate with which spatial correlation decays
with distance (Figure \@ref(fig:sdmTMB-matern-demo)). The \mbox{Mat\'ern}
function describes the covariance between spatial locations $s_j$ and $s_k$ as:
...
approximation to Gaussian Markov Random Fields (GMRF) [@lindgren2011] to
...
of knots than the full spatial data set, we can greatly increase computational
...
matrix can be calculated. Higher numbers of knots result in a better
approximation of the special random effects at a greater computational cost. We
fit the spatial models with one less knot than the number of observations if
there were greater than 200 observations and 200 knots otherwise. Following
common practice [e.g. @thorson2017b; @anderson2018] we chose the location of
the knots with a k-means clustering algorithm with a fixed random seed to
...
\@ref(fig:sdmTMB-depth-all-plots1), \@ref(fig:sdmTMB-depth-all-plots2)). These
plots provide some indication about which surveys encompass the entirety of the
depth distribution for particular species.
...
```{r sdmTMB-depth-eg, eval=FALSE, fig.width=6, out.width="4in", fig.cap='Predicted relationship between depth and biomass density for Pacific Cod based on the quadratic depth predictors. These are predictions for a mean spatial location effect.'}
...
ggplot(out, aes(depth, y)) + geom_line() +
  coord_cartesian(xlim = c(-300, 0), expand = FALSE,
    ylim = c(0, max(exp(y))*1.04)) +
  labs(x = 'Depth (m)', y = expression(Biomass~density~(kg/km^2)))
```

```{r sdmTMB-depth-all-calculations, eval=TRUE}
...
make_depth_plot <- function(.data) {
  ggplot(.data, aes_string('depth', 'y', colour = 'survey')) +
    ...
    labs(x = 'Depth (m)',
      y = expression(Biomass~density~(kg/km^2)), colour = 'Survey') +
...
```{r sdmTMB-depth-all-plots1, fig.width=10, fig.asp=1.2, out.width="\\textwidth", fig.cap='Prediction relationships between depth and biomass density for all species across the four synoptic surveys (Part 1 of 2). Dashed lines indicate extrapolated relationships beyond the observed depth range of the survey.'}
...
```{r sdmTMB-depth-all-plots, fig.width=10, fig.asp=1.3, out.width="\\textwidth", fig.cap="(ref:depth-relationship2)"}
