...
g_depth <- plot_map(grid_locs, "depth") +
  scale_fill_viridis_c(trans = "fourth_root_power", option = "A", direction = -1) +
  ggtitle("Depth") +
  labs(fill = 'Depth (m)')
```
...
g_sdm2 <- plot_map(predictions$data, "exp(est_fe)") +
  ...
  scale_fill_viridis_c(trans = "fourth_root_power", option = "D") +
  labs(fill = 'Biomass estimate\nkg/km^2')
```
