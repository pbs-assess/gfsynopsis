...
GLMMs (generalized linear mixed-effects models) with a log link:
...
function, which simply defines the shape with which spatial correlation decays
...
centred and scaled variable. This ensures the covariate values are not too big
or small to avoid computational issues and the centering separates the linear
and quadratic predictor components.
...
various surveys were conducted in.

TODO:

- how many knots and why
- what the triangular mesh shows
- the projection matrix from INLA
...
```{r sdmTMB-spde, fig.width=9, out.width="3.7in", fig.cap="Example triangularization mesh for Pacific Cod and the Queen Charlotte Sound survey in 2017. The red dots indicate knot locations. The open grey circles in the background represent the locations of the observed data. The lines show the triangularization mesh used in the SPDE approximation. In this case, many of the knots overlap the observed data. A greater number of knots will increase the accuracy of the approximation at the expense of computational time."}
...
```{r sdmTMB-depth-eg, eval=TRUE, fig.width=6, out.width="4in", fig.cap='Predicted relationship between depth and biomass density for Pacific Cod based on the quadratic depth predictors. These are predictions for a mean spatial location effect.'}
...
```{r sdmTMB-maps-combined, fig.width=9, fig.asp=1, out.width="6in", fig.cap="Example spatial model components in Queen Charlotte Sound for Pacific Cod. (Top left) Bottom depth data that is used as a predictor. (Top right) Predicted biomass density from the quadratic effect of depth only. (Lower left) Spatial random effect deviations. These deviations represent on modelled correlated spatial effects. (Lower right) Model predictions that include both the depth fixed effects and the spatial random effects."}
