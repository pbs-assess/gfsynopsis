...
grid_locs <- gfplot:::make_prediction_grid(filter(dat, year == 2017),
  survey = "SYN QCS", cell_width = 2)$grid
grid_locs <- rename(grid_locs, depth = akima_depth)
grid_locs$year <- NULL
```

```{r sdmTMB-spde, fig.width=9, out.width="4in", fig.cap="TODO"}
...
```{r sdmTMB-resids, fig.width=6, out.width="4in", fig.cap="TODO"}
...
```{r sdmTMB-preds, fig.width=6, out.width="4in", fig.cap="TODO"}
...
```{r sdmTMB-plot, fig.width=6, out.width="4in", fig.cap="TODO"}
plot_map(predictions$data, "exp(est)") +
  scale_fill_viridis_c(trans = "sqrt", option = "C") +
  ggtitle("Prediction (fixed effects + spatial random effects)")

```

```{r sdmTMB-plot-fe, fig.width=6, out.width="4in", fig.cap="TODO"}
plot_map(predictions$data, "exp(est_fe)") +
  ggtitle("Prediction (fixed effects only)") +
  scale_fill_viridis_c(trans = "sqrt", option = "D")
```

```{r sdmTMB-plot-re, fig.width=6, out.width="4in", fig.cap="TODO"}
