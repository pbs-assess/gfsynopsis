...
<<make_iphc_dat>>=
# Tried extracting some code from make.R and from various functions, but
#  should really make a function just for IPHC maps - going to take too long.
dc <- here("report", "data-cache")
sp <- "redbanded rockfish"
sp_w_hyphen <- "redbanded-rockfish"
dat_iphc <- readRDS(file.path(dc, paste0("iphc/", sp_w_hyphen, ".rds")))
...
iphc_map_dat$akima_depth <- mean(iphc_fits$raw_dat$depth,
        na.rm = TRUE)
    iphc_map_dat$combined <- ifelse(iphc_map_dat$combined ==
        0, NA, iphc_map_dat$combined)
    suppressMessages({
        g_survey_spatial_iphc <- gfsynopsis::plot_survey_maps(iphc_map_dat,
            iphc_fits$raw_dat, show_raw_data = FALSE, cell_size = 2,
            circles = TRUE, show_model_predictions = "combined" %in%
                names(iphc_map_dat), annotations = "IPHC") +
            coord_cart + ggplot2::ggtitle("IPHC survey catch rate") +
            ggplot2::scale_fill_viridis_c(trans = "fourth_root_power",
                option = "C", na.value = "white") + ggplot2::scale_colour_viridis_c(trans = "fourth_root_power",
            option = "C", na.value = "grey35")
    })
@
