...
\author{A reproducible data synopsis for over 100~species
  of British Columbia groundfish\\
  ~\\
  Sean Anderson, Elise Keppel and Andrew Edwards\\
  ~\\
...
\subtitle{\small CSAP review meeting\\
...
Longest continually running groundfish survey in BC waters:\\
\bi
  \item annual stock assessment longline survey, California to Alaska
  \item main aim is data for Pacific Halibut assessment
  \item  each station has a set of \textcolor{red}{up to} 8 skates
  \item  each skate has \textcolor{red}{about} 100 hooks
  \pause
...
\pause
1997-1998 & \textcolor{red}{First 20 of each skate} & Set-by-set & Spreadsheet$^2$ & N\\
\pause
1999 & \textcolor{red}{First 20 of each skate} & Set-by-set & Spreadsheet$^2$ &
                                                                                \textcolor{blue}{{\bf Y}}\\
\pause
2000 & \textcolor{red}{First 20 of each skate} & Set-by-set & Spreadsheet$^2$ & N\\
\pause
2001-2002 & \textcolor{red}{First 20 of each skate} & Set-by-set & Spreadsheet$^2$ & \textcolor{blue}{\bf Y}\\
\pause
2003-2011 & All & \textcolor{purple}{Hook-by-hook} & DFO database GFBio & \textcolor{blue}{\bf Y}\\
\pause
2012 & All (\textcolor{green}{bait experiment})      & \textcolor{purple}{Hook-by-hook} & DFO database GFBio & \textcolor{blue}{\bf Y}\\
\pause
2013 & \textcolor{red}{First 20 of each skate} & Set-by-set & Spreadsheet$^3$ & \textcolor{blue}{\bf Y}\\
\pause
2014-2017 & All & \textcolor{purple}{Hook-by-hook} & DFO database GFBio & \textcolor{blue}{\bf Y}\\
   \hline
\end{tabular}
\end{table}
...
1996 & All & Set-by-set & Spreadsheet$^2$ & N\\
1997-1998 & \textcolor{red}{First 20 of each skate} & Set-by-set & Spreadsheet$^2$ & N\\
1999 & \textcolor{red}{First 20 of each skate} & Set-by-set & Spreadsheet$^2$ & \textcolor{blue}{\bf Y}\\
2000 & \textcolor{red}{First 20 of each skate} & Set-by-set & Spreadsheet$^2$ & N\\
2001-2002 & \textcolor{red}{First 20 of each skate} & Set-by-set & Spreadsheet$^2$ & \textcolor{blue}{\bf Y}\\
2003-2011 & All & \textcolor{purple}{Hook-by-hook} & DFO database GFBio & \textcolor{blue}{\bf Y}\\
2012 & All (\textcolor{green}{bait experiment})      & \textcolor{purple}{Hook-by-hook} & DFO database GFBio & \textcolor{blue}{\bf Y}\\
...
\frame{\frametitle{Can create different series}
\begin{columns}
    \begin{column}{0.48\textwidth}
        Series A -- north of VI, first 20 hooks \\
        \includegraphics[width=6cm, left]{../../report-rmd/figure/andy-orig/bcaKeepSerA.png}
    \end{column}
    \begin{column}{0.48\textwidth}
        ~~~~~~~~~~Series B -- north of VI, all hooks \\
        \includegraphics[width=6cm, right]{../../report-rmd/figure/andy-orig/bcaKeepSerB.png}
    \end{column}
\end{columns}
}
...
\frame{\frametitle{Rescale to join up}
\begin{columns}
    \begin{column}{0.48\textwidth}
        Divide each series by its geometric mean \\
        \includegraphics[width=6cm, left]{../../report-rmd/figure/andy-orig/bcaKeepSerAB1.png}
    \end{column}
    \begin{column}{0.48\textwidth}
        ~~~~~~~~~~Rescale B to join up to A \\
        \includegraphics[width=6cm, right]{../../report-rmd/figure/andy-orig/bcaKeepSerAB2.png}
    \end{column}
\end{columns}
}
...
\frame{\frametitle{Construct four different time series}
...
\bi
  ...
  \item Series B and C
\ei
to determine the longest that can be made (usually A with extra rescaled years from B).

~\\

Show longest series in the report.
}

% ----------------------------------------------------------

\frame{\frametitle{Hook competition}
\bi
  \item competition by fish for the bait
  \item if a hook catches a fish then that hook is no longer actively fishing
    for the remainder of the soak time
  \item so true local density of a species should be scaled up depending on the
    number of hooks returned without bait
\ei
}
...
\frame{\frametitle{Hook competition}
Based on approach used by IPHC, adjustment factor would be:
<<hookcomp, fig.height=2.5>>=
...
plot(Pit, hook_adjust, ylim=c(0, max(hook_adjust)), type="l",
     xlab = expression("Proportion of hooks with bait"),
     ylab = "Adjustment factor")
@
~\\
But need to decide what to do when no hooks are returned with bait.

Happens for 646/3434 sets based on first 20 hooks.

Likely needs to be a separate project.
}
