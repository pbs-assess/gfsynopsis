...
\end{tabular}
\end{table}
Statistically compare Series A and B; Series A and D; Series B and C to
determine the longest that can be made.
...
\bi
  ...
  \item if a hook catches a fish then that hook no longer actively fishing
