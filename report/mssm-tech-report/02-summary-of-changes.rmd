...
# Summary of changes to the MSSM survey and implications for groundfish indices
...
Changes in spatial coverage over time may mean that an index estimated using a spatiotemporal model is better suited for calculating an index over the full time-series than a design-based index. <!-- TRUE??? -->
...
No calibration tows were performed, but @boutillier1976cruiserep found that the NMFS net was 1.4 more efficient than the 70 foot semi-balloon net, meaning that catch weights in the first years of the survey are likely to be biased down relative to 1977 onward.
...
Unfortunately, the lack of calibration tows means that it is not possible to include gear changes in a modelled index. 
Furthermore, the visual comparison of only two comparative tows does not provide a clear indication of how the gear change affected capture rates.
