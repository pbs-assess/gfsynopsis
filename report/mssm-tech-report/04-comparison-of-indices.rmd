...
The Small-mesh Multi-species survey is the longest running continuous bottom trawl survey on the BC coast.
As a long-running dataset for groundfish, it is therefore important to understand how well the data capture trends in species abundances.
...
The CPUE 3CD data locations are shown as 7 km wide bins that implements the 3-vessel privacy requirement.
The solid hexagons are from 2013 onwards after the trawl footprint was frozen, hexagons outlined in grey to illustrate fishing prior to the frozen footprint (i.e., 1996--2012). 
...
The design-based indices were calculated following a standard depth-stratified mean estimator.
For each year, an average catch was calculated for each of two depth strata and then multiplied by each stratum's area. 
Confidence intervals were estimated using a bootstrap procedure with 1000 replicates, drawing stratified samples with replacement, from all tows [see Appendix F in @anderson2019synopsis].
...
For the SMMS, the sampling domain was defined by creating a 3 $\times$ 3 km grid covering the extent of all survey locations in WCVI Shrimp Survey Areas 124 and 125.
...
For the SYN WCVI, we compared indices predicted on the 2 $\times$ 2 km grid covering the extent of the SYN WCVI sampling domain, but also on the 3 $\times$ 3 km SMMS grid. 
...
```{r mssm-syn-wcvi, fig.cap="Comparison of SMMS (green) and SYN WCVI (purple) indices. Time-series with a solid trend line and shaded ribbon for 95\\% confidence intervals represent an index that has been standardised with a spatiotemporal model. Vertical axes are scaled between zero and the maximum upper confidence interval value of the modelled index. Then the two indices have been scaled to have the same geometric mean in overlapping years. The axes are set to encompass the confidence intervals of the SMMS index. The SYN WCVI data begin in 2004. Empty panels indicate species that we do not have a model for in the SYN WCVI data."}
knitr::include_graphics(here::here("report/mssm-tech-report/figure/index-mssm-model-syn-wcvi-model.png"), dpi = 300)
```

```{r mssm-syn-wcvi-mssm, fig.cap="Comparison of SMMS (green) and SYN WCVI (purple) indices where the SYN WCVI indices have been estimated from model predictions done on the SMMS grid. Time-series with a solid trend line and shaded ribbon for 95\\% confidence intervals represent an index that has been standardised with a spatiotemporal model. Vertical axes are scaled between zero and the maximum upper confidence interval value of the modelled index. Then the two indices have been scaled to have the same geometric mean in overlapping years. The axes are set to encompass the confidence intervals of the SMMS index. The SYN WCVI data begin in 2004. Empty panels indicate species that we do not have a model for in the SYN WCVI data."}
...
```{r rolling-window-correlation, fig.cap="Ten-year rolling window correlations of the modelled SMMS index with the SYN WCVI predicted on the SMMS grid, the SYN WCVI predicted on the SYN WVCVI grid, and with the CPUE 3CD. The x-axis is the first year of the rolling window (e.g., 1996--2006) and the y-axis is the correlation over the 10-year window. The thick blue line is the average correlation of all species over time. Only shown are species that had a correlation $\\leq$ 0.5 when surveys were compared over the full time series. Years before 2003 are shaded grey to indicate the time before complete sorting and species level identification.", out.width = "100%"}
