...
The Multispecies Small-Mesh Survey is the longest running groundfish survey on the BC coast, therefore it is important to understand how well the historical data capture trends in species abundances.
...
The design-based indices are calculated as the mean catch weight in each depth stratum (which for shrimp is a single stratum??). <!-- Do you know if this is true - that shrimp  are just one depth stratum? The survey is only ~50 - 150 m -->
This mean catch is then multipled by the total stratum area. 
Confidence intervals are estimated using a bootstrapping procedure with 1000 replicates, drawing samples with replacement, from all the tows (See Appendix F in anderson2019synopsis).

For the modelled MSSM indices, SYN WCVI indices, and commercial CPUE 3CD indices, we used the model outputs from the 2024 Technical Report by @anderson2024synopsis.  

Briefly, spatiotemporal models were fit to the data from the MSSM and the synoptic trawl survey off WCVI [SYN WCVI] using the R [r2023] package `sdmTMB` [@anderson2022sdmTMB]. 
...
The spatiotemporal models for both the MSSM and SYN WCVI had constant spatial random fields. 
Unless models did not converge, then the spatial random field was turned off (i.e., set to zero) and only the spatiotmporal random field was included.
The spatiotemporal component however differed between the surveys, for the MSSM indices, the models were structured using a random walk with Gaussian field annual deviations, to account for the changes in the sampling domain over time.
For the SYN WCVI, which has a consistent sampling domain over time, the models were structured using independent and identically distributed (IID) Gaussian field annual deviations.
For each survey and species, five possible families were considered: Tweedie, delta-gamma, delta-lognormal, delta-Poisson-link-gamma, and delta-Poisson-link-lognormal.
...
We omitted poorly fitted models: those which did not meet convergence criteria, those where the maximum estimated CIs were greater than 10 times the maximum estimated biomass, and those where the mean coefficient of variation was < 1. Details are available in Appendix A of Anderson et al. (@anderson2024synopsis).

<!-- Need blurb about CPUE 3CD -->

To derive a area-weighted biomass indices, the models were then used to estimate abundance across standardised sampling grids.
...
We used a subset of the grid cells that were last sampled between 2009 to 2021 (Figure \@ref(fig:prediction-grid)).
