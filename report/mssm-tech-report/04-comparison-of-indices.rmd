...
The Multispecies Small-Mesh Survey is the longest running continuous groundfish survey on the BC coast, therefore it is important to understand how well the historical data capture trends in species abundances.
...
The spatiotemporal models for both the MSSM and SYN WCVI surveys were first with constant spatial Gaussian Markov random fields (GMRFs) and spatiotemporal GMRFs. 
...
The spatiotemporal component however differed between the surveys, for the MSSM indices, the GMRFs were structured as a random walk to account for the changes in the sampling domain over time.
...
For each survey and species, five possible families were considered: Tweedie, delta-gamma, delta-lognormal, delta-Poisson-link-gamma, and delta-Poisson-link-lognormal [@anderson2024synopsis].
...
Locality (spatial polygon), a locality-year intercation, and fishing vessel are treated as random intercepts.
...
The average correlation between the MSSM and the CPUE 3CD indices was lower in the years prior to 2003 than after, which is driven by low correlations in Bocaccio and Redstripe Rockfish in early years. When the full time-series were compared, only 10/11 of the species had correlations $\ge$ 0.5. 
