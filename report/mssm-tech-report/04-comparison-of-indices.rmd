...
All of the species captured in the MSSM survey are also caught in the SYN WCVI (Figure \@ref(fig:encounter-tigure))

```{r encounter-tigure, fig.cap="Encounter probability and mean density of species caught on "}

knitr::include_graphics(here::here("report/mssm-tech-report/figure/mssm-wcvi-tigure.png"), dpi = NA)

```



Twenty-four species have length samples in both the MSSM and SYN WCVI. Longer specimens tend to be sampled in the SYN WCVI compared to the MSSM (Figure \@ref(fig:length-dist))

There was also no relationship between the


```{r length-dist, fig.cap="Distribution of sampled length distributions for species observed in the MSSM (green) and SYN WCVI (purple) surveys. Panels separate species based on the relative difference in mean observed size between the surveys and species are ordered from left to right in increasing differences in mean length."}
