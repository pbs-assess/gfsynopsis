...
The SMMS indices correlate well with the SYN WCVI in overlapping years from 2003 onwards for many species.
...
It is unclear whether this is due to fisheries dependent vs. independent data, different standardization methods, different spatial domains, or changes to the SMMS survey from 2003 onwards in catch sorting and sampling.
Regardless of whether the SMMS is used for years pre-2003, it may be useful to assessments because it provides an annual index instead of biennial and tends to sample the same or smaller size groundfish, presumably due to a smaller mesh size.
As of 2024, there are plans to revise the SMMS survey design.
If the survey design is revised, future review of the impacts on groundfish may be required.
