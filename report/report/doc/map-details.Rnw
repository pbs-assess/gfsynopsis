...
At a high level, these models model the expected biomass in space as a
continuous process with a quadratic effect for bottom depth and a residual
`random effect' wiggly surface that represents an amalgamation of other spatial
processes not explicitly included in the model. This random effect surface
allows for the fact that fish biomass should be more related for points in space
closer together and less related for those further away. After fitting the model
to sets from trawl or longline surveys the model is then projected to a
fine-scale grid over the entirety of the survey domain to drive estimates of
...
combine a model explaining the probability of presence of a species in a tow
with a model explaining the expected biomass in a tow given that it has been
...
Details to follow on the implementation. See
