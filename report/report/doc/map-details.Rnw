We model the expected biomass density in space for each species using
...
We fit binomial occurrence models (denoted with a superscript $B$), which
...
GLMM with a similar structure to the spatial random effects:
...
similar spatial models \citep[e.g.][]{ward2015, thorson2015, ono2016, shelton2018}, which
lets the covariance function be more flexible than an exponential covariance
function but still be differentiable \citep{rasmussen2004}.
...
We fit the spatial models using the package INLA \citep{rue2009, lindgren2015}
for the statistical software R \citep{rcoreteam2018}, which \ldots
