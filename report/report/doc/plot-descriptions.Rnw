\emph{TODO: A brief introduction to this section and the layout in general.}

\begin{resdoclist}

\item Colours are consistent for the surveys across the survey biomass index
  trends and the age and length compositions to facilitate tracing a single
...
\item Female data features are always shown in front of male data features and
  are either coloured or black whereas males are always indicated with light
  gray.

\item We have attempted to focus on visualizations, model fits, and survey
  selections that are applicable to the widest array of groundfish stocks. For
  example, we are not showing biomass index trends or maps from the highly
  selective sablefish trap surveys, although we include biological
  sample fish specimen counts across all surveys.
...
<<catches, fig.asp=1.3, fig.width=5, out.width="4in", warning=FALSE, fig.cap=paste0("Example commercial fishery catches plot for ", sp, ". Catch from various gear types is indicated by color shading. Catch is calculated as the weight of landings plus discards aggregated by year; however, hook and line discards are mostly exluded from these data. Years before 1996 and 2006 are shaded grey to indicate that they are considered less reliable.")>>=
g$catch
@
...
<<cpue-maps, fig.asp=1.47, fig.width=4.1, out.width="3.4in", fig.cap=paste0("Example commercial trawl and hook and line catch per unit effort maps for ", sp, ". Lighter shading indicates higher levels of a geometric mean of catch per unit effort in a given hexagon cell. Cells are 7km wide and are only shown in cases where there are at least 3 unique vessels in given cell for privacy reasons. For bottom trawl, catch per unit effort is calculated as the weight of catch (landings plus discards) divided by hours fished for all positive tows from the groundfish trawl sector. Trawl data are shown from 2013 onwards when the trawl footprint was frozen. For hook and line, catch per unit effort is shown as the number of fish recorded landed and discarded and so is more accurately considered the number of fish caught. Hook and line data are shown from 2008 onwards. ", map_text)>>=
gridExtra::grid.arrange(
...
<<length-weight-vb, warning=FALSE, fig.asp=0.4, fig.cap=paste0("Example length-age and length-weight model fits and plots for ", sp, ". The length-age growth curve is a von Bertalanffy model (both models described further in text). Female model fits are indicated as solid black lines and male model fits are indicated as dashed grey lines. Text on the panels shows the parameter estimates and light dots represent individual fish that the models are fit to.")>>=
...
The von Bertalanffy growth models were fit with Stan as:
...
for a given stock. These weakly informative priors helpful when fitting these
nonlinear models to a large number of stocks where maximum likelihood methods
can give nonsensical results for some stocks and it is difficult to adjust the
optimization procedure on a stock-by-stock basis.

We fit the length-weight models as robust linear regressions of log(length) on
log(weight) using the \texttt{MASS::rlm()} function. This down weights the
influence of outlying values and helps generate reasonable faith across all
species without handpicking outlying measurements to discard.
