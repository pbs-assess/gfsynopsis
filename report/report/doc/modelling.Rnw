...
same design-based bootstrap approach used in recent BC groundfish stock
assessment reports. Details to follow.

\subsection{SURVEY METHODS} \label{sec:survey-methods}

Survey design and methods for the various surveys referenced in this report can
be found in the following documents:

\begin{resdoclist}

\item Synoptic Survey, Queen Charlotte Sound -  \citep{williams2018synqcs}

\item Synoptic Survey, West Coast Vancouver Island - \citep{williams2017synwcvi}

\item Synoptic Survey, Hecate Strait - \citep{nottingham2018synhs}

\item Synoptic Survey, West Coast Haida Gwaii - \citep{nottingham2018synwchg}

\item Hardbottom Longline Survey, Inside - \citep{lochead2004irf}

\item Hardbottom Longline Survey, Outside - \citep{dfo2018ifmp}

\item International Pacific Halibut Commission - \citep{flemming2012iphc}

\item Hecate Strait Multispecies Assemblage Survey - \citep{fargo1984hsmsa};
  \citep{westrheim1984hsmsa}
...
and a second to data representing the CPUE conditional on a tow having caught
...
recorded at least 4 positive trips in at least 4 years since 1996. These
...
\begin{equation}
  \log (y_i^L) \sim \mathcal{N}(\bm{X_i^L} \bm{\beta^L}, \sigma),
\end{equation}
...
\citep{maunder2004}. For example, we set the reference month as the most
...
representative sampling in sufficient sample sizes.
