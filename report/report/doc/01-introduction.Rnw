...
groundfish species in British Columbia. However, we (DFO Science Branch, Pacific
...
regularly published or readily accessible. As one step to address this problem,
we have created a data synopsis report to give a snapshot of long-term and
...
is an extension of the data scorecard concept discussed at a Canadian Science
Advisory Secretariat (CSAS) Regional Peer Review Meeting in May 2016
\citep{macdoughall2016}. We plan to publish this report as a CSAS Research
Document in its first year and to update the report annually as a CSAS Science
...
stitching the document together to facilitate rapid publication.
...
takes inspiration. We also think Norm Olsen for his work on GFASTeR, which parts
