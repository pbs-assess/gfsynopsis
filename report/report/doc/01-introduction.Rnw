...
<<ss-maps, fig.height=2, fig.width=7.9, out.width="\\textwidth", fig.cap="Synoptic bottom trawl survey boundaries (left) and Outside Hard Bottom Long Line survey boundaries (right). The colours matches the colour coding through the rest of the report.", fig.pos="htbp">>=
library(ggplot2)
library(gfplot)
library(dplyr)
xlim <- c(122, 890)
...
cols <- paste0(RColorBrewer::brewer.pal(4, "Set1"), "80")
...
cols <- paste0(RColorBrewer::brewer.pal(7, "Set1"), "80")

g2 <- ggplot()
g2 <- g2 + geom_rect(data = hbll,
  aes_string(xmax = "X + 1", ymax = "Y + 1", xmin = "X - 1", ymin = "Y - 1", fill = "survey")) +
  scale_fill_manual(values = c(
    "Outside Hard Bottom Long Line (N)" = cols[6],
    "Outside Hard Bottom Long Line (S)" = cols[7])) +
  geom_path(
  data = isobath_utm, aes_string(
    x = "X", y = "Y",
    group = "paste(PID, SID)"
