...
analyses for which they will be used. The SQL file names mentioned in the
...
We extract commercial catch with \texttt{get-catch.sql}. All landings and
discards are extracted by species, fishery sector, gear type and year, and are
not filtered in any further way.

We extract commercial trawl catch per unit effort (CPUE) index data using
...
We extract commercial trawl spatial CPUE data using
...
We extract commercial hook and line spatial CPUE data using
...
We extract survey biomass index data \texttt{get-survey-index.sql}. Calculated
...
We extract biological data using \texttt{get-survey-samples.sql} and
...
% All samples are extracted and can be filtered to allow analysis on all samples
...
In the synopsis report, we are only including unsorted biological samples.
