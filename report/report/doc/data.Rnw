...
combinations of two codes relating to sampling procedures:
\texttt{SPECIES\_CATEGORY\_CODE} and \texttt{SAMPLE\_SOURCE\_CODE}.
Samples can be designated as `unsorted samples' or `sorted samples' (where
selected samples are sorted into `keepers' and `discards').
All samples are extracted and can be filtered to allow anaylysis on all samples
or on only sorted samples to remove bias from inclusion of sorted samples.
...
scc <- readr::read_csv("../tables/spp-category.csv")
print(xtable::xtable(scc, caption = "Species category codes lookup table which describes sampling protocols at the catch level"), include.rownames = FALSE,
  floating = TRUE, booktabs = TRUE, caption.placement = "top")
@

<<sample-source-code-table, message=FALSE, warning=FALSE, results='asis'>>=
ssc <- readr::read_csv("../tables/sample-source.csv")
print(xtable::xtable(scc, caption = "Sample source codes lookup table which describes sampling protocols at the sample level"), include.rownames = FALSE,
  floating = TRUE, booktabs = TRUE, caption.placement = "top")
@
