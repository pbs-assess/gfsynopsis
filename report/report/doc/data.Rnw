...
Functions were developed within the \textbf{gfplot} package to automate
...
analyses they will be used in.
The extracted data are formatted to feed directly into other functions in the
package to allow for a fully automated procedure.
...
and year and are not filtered in any way (Table~\ref{tab:sql-no filters}).
Species names or codes are specified as an argument to the function by the user.
...
records with hook and line gear with non-zero catch and since 2008 after
implementation of the integrated groundfish management plan
(Table~\ref{tab:sql-cpue-spatial-ll}).
CPUE is represented by landed catch in pieces per fishing event (set). Discards
are not included in hook and line spatial CPUE as discarded pieces are not
reliably recorded.
Species names are given as an argument to the function.
...
based on combination of \texttt{SPECIES\_CATEGORY\_CODE} and
...
all samples.
Samples are also filtered by \texttt{SAMPLE\_TYPE\_CODE} for only total or
random samples.
...
Filtered for \texttt{FISHERY_SECTOR} = \texttt{GROUNDFISH TRAWL}                                                   & To extract only records in the groundfish trawl fishery                   \\
Filtered for \texttt{ISNULL(LANDED_ROUND_KG,0) + ISNULL(TOTAL_RELEASED_ROUND_KG,0)} > 0                            & To extract only records with positive catch                               \\
\bottomrule
\end{tabu}}
\end{table}
...
Filters                                                                                                            & Rationale                                                                \\
\midrule
Filter for \texttt{AGE_READING_TYPE_CODE} 2, 3                                                                     & To extract primary and precision test readings                           \\
\bottomrule
\end{tabu}}
\end{table}
