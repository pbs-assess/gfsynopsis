...
```{r sdmTMB-spt-spde, fig.cap='Exemple de maillage de triangularisation pour la morue du Pacifique et le relevé du bassin de la Reine-Charlotte au fil des ans. Les points rouges représentent les emplacements des nœuds et les cercles noirs ouverts représentent l’emplacement des ensembles de relevés.', fig.width=9, out.width="3.7in", eval=FALSE}
spde <- make_spde(dat$X, dat$Y, n_knots = n_knots)
plot_spde(spde)
```

```{r sdmTMB-spt-model, warning=FALSE, message=FALSE, eval=FALSE}
m_st <- sdmTMB(
...
```{r sdmTMB-spt-model-depth, eval=FALSE}
m_st_depth <- sdmTMB(
...
```{r sdmTMB-spt-residuals, fig.cap='Histogramme des résidus de quantiles aléatoires.', eval=FALSE}
...
```{r sdmTMB-spt-plot-function, eval=FALSE}
...
```{r sdmTMB-spt-plot-all-effects-sqrt, fig.cap='Prédictions tirées d’un modèle spatiotemporel géostatistique pour la morue du Pacifique dans le bassin de la Reine-Charlotte. La biomasse prévue est représentée par une échelle de couleurs répartie sur une racine quatrième', eval=FALSE}
...
```{r sdmTMB-spt-spatial-residuals, fig.cap='Résidus spatiaux tracés au fil du temps sur l’échelle des liens (logarithme).', eval=FALSE}
