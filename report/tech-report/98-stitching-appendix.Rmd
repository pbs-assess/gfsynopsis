...
We used spatiotemporal models to combine or "stitch" related surveys into coastwide or survey-wide indices of abundance or biomass.
These indexes appear in the survey relative index figures (e.g., Figure \@ref(fig:survey-index)) for IPHC FISS, HBLL INS N/S, SYN WCHG/HS/QCS/WCVI, MSSM WCVI, and HBLL OUT N/S.

We can represent the spatiotemporal model as
...
The offset is effectively a covariate with a coefficient fixed at 1 and allows us to control for survey effort (here, log area swept or log effective hooks fished).
...
We explored a censored Poisson likelihood with additional observation-level dispersion [@watson2022], but found a poorer fit to the data (as indicated by conditional AIC (Akaike information criterion) and plots of residuals) for many species and model convergence issues in several cases.
...
We constructed our meshes with a minimum triangle edge length of 20 km for the SYN, HBLL, and IPHC models and with a minimum edge length of 5 km for the MSSM WCVI survey, which was smaller in size.
...
For the MSSM survey we also omitted models when the maximum estimated confidence intervals were $\ge$ 10 times the maximum estimated biomass or when the mean coefficient of variation was greater than 2.
...
```{r mssm-grid, fig.cap="Prediction grid used to calculate biomass indices for the Multispecies Small-Mesh Bottom Trawl Survey (MSSM). Grid cells are 3 $\\times$ 3 km and cover all sampling locations 2009--2021. Points (grey) indicate sampling locations, with points in black representing locations sampled 2009--2021.", out.width="4in"}
