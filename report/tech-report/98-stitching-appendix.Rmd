...
For the hook and line surveys (IPHC, HBLL OUT, HBLL INS), we applied an "instantaneous catch rate" ICR correction factor to the observed counts based on the number of baited hooks returned (REF).
We explored a censored Poisson likelihood (REF), but found a poorer fit to the data and model convergence issues.
We plan to revisit this in a future version of the report.

We implemented these models using the Stochastic Partial Differential Equation (SPDE) approximation to Gaussian random fields using Gaussian Markov random fields for computational efficiency using the R (REF) package sdmTMB (REF).
This SPDE approach requires creating triangulation "meshes" (REF).
...
From our fitted models, we calculated an area-weighted biomass index by predicting density from our model across the entire survey grid, multiplying that density by grid cell area, and summing the resulting biomass across grid cells.
