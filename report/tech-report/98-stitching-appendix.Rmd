...
Such a structure has been used previously, for example, in REF, REF, and [@english2023].
It is also similar to the first-order autoregressive (AR1) structure used by the International Pacific Halibut Commission when modelling their survey data (REF).
...
We explored a censored Poisson likelihood [@watson2022], but found a poorer fit to the data (as indicated by log likelihood and residuals) for many species and model convergence issues in several cases.
...
For the MSSM survey we also omitted models when the maximum estimated confidence intervals were $\ge$ 10 times the maximum estimated biomass and the mean coefficient of variation was less than 2.
