...
The navigation systems used in this survey have changed twice, once in 1979 from Loran A to Loran C, and again in 1998 from Loran C to the Global Positioning System (GPS) (Figure \@ref(fig:historical-grid)). This likely means that early survey locations were more variable. However, it was also found that there there was a trend in distance travelled over time. With shorter distances travelled for the 30 minute tows once GPS was implemented (Sinclair et al 2001).
...
The gear used in the survey has changed twice in

```{r net-comp, fig.cap="2006 gear change comparison", out.width="50%"}
...
From 2003 to present, all species have been sorted and identified to the species or lowest taxonomic level.
...
To check if some species were identified at higher taxonomic levels prior to 2003, we extracted data from GFBio for higher order taxonomies of species that have been identified in the MSSM survey since 2003 (\@ref(fig:spp-aggregate-plot)).
Sculpins and skates are the main groups that were counted at the family level prior to 2003.
While flatfishes and rockfishes were most often identified to species if they were counted/weighed.
...
We fit a geostatistical model to the MSSM data for all species that had a proportion of positive sets $\ge$ 5%.
For the six species that do not have catch weights before 2003, we calculated the proportion of positive sets for 2003 to present.
...
We fit survey data using a spatial random field and a random walk spatiotemporal random fields.
...
Not all models fit the MSSM, this plot is the index only, but these are also shown in the individual species panels so I don't know if you want this here as well.
But not sure if this should be a bit stand alone so people don't have to flip back and forth
...
```{r, echo=FALSE, eval=TRUE, tab.cap="Change table", out.width='\\textwidth'}
