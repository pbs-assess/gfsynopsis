...
Dots represent mean estimates of relative biomass and vertical lines around the dots represent 95% bootstrap confidence intervals.
Time series with a solid trend line and shaded ribbon for 95% confdence intervals represent an index that has been standardized with a geostatistical model (see Appendix A) \@ref(app:stitching).
...
When both indices are shown, they are scaled to have the same geometric mean.
`r surv_abbrev_text`.
Also included are coastwide or survey-wide indices for SYN WCHG/HS/QCS/WCVI, HBLL OUT N/S, and HBLL INS N/S.
For the HBLL OUT, HBLL INS, and IPHC FISS the values are relative counts (per effective skate for IPHC FISS) rather than biomass.
For the MSSM WCVI, prior to 2003 groundfish sorting and species-level identification was not systematic and so this time period has been shaded grey.
...
`r sp`). Lighter shading indicates higher levels of a geometric mean of catch per
...
intervals to facilitate tracing cohorts through time. See Figure
\@ref(fig:survey-index) for survey abbreviations. Ageing precision plots
