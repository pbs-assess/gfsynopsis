...
Such an approach facilitates new research ideas---such as correlations that might exist between biological trends and populations, causes of change in species index trends, or relationships between fishery trends before and after management changes---and addresses questions such as:
...
3. Maximize information that could be presented in the limited space *by* focussing on data types relevant to the most species,
...
Figure 4: Data synopsis report "data-to-document" workflow. A: Illustration of the (automated) flow through data-extraction to visualization-plotting to report production for a data synopsis report. B: Examples of workflow through some of the modular functions the authors built and employed to ensure transparency, traceability, and reproduciblity. Shown are functions for extracting biological data from commercial and fishery-independent surveys, which are then passed to functions for tidying and arranging the data and/or fitting to statistical models prior to plotting visualizations.
