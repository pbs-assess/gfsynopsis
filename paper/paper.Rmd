...
- there is an increasing need to frequently monitor stocks and across a large swath of dimensions given the rapid environmental change and increased variability in general reduced predictability associated with climate change

Thankfully, the advent of modern sensor technologies, relational databases, automated reporting, drones, etc. has resulted in vast quantities of data that can aid with this monitoring, but the data often remains hidden either because of permission or privacy issues or just through obscurity.
- give some examples of monitoring programs from across Canada and around the world
- give an example of privacy issues
...
- focus on fisheries independent surveys that . . .
- maintain a consistent layout for all species
- limit ourselves to two pages per species so that we can look at all data for one species that once
- highlight uncertainty wherever possible
- assemble in display metadata alongside visualization
- maintain consistent colour meeting throughout and make it colourblind proof where possible
- group similar focused visualizations together
- focus on both spatial and temporal aspects of the data
- focus on aspects of the data that are most important to stock assessment etc.
...
- Show empty plots to emphasize missing data and maintain the same layout for all species

How we did it:
- develop a family of R packages the facilitate going directly from our relational databases, two extracting necessary data, to tidying the necessary data, two fitting statistical models, and to visualizing the data and statistical models in a report format
- the entire report had to be fully automated and reproducible so that it could be easily updated in the future
- write the code for the report in a modular way (R packages) so that bits and pieces could be useful for other purposes, for example in assessments

We settled on visualization components that covered trends in time and space for fisheries independent surveys, trends in time and space for fisheries catch and catch per unit effort. Visualizations of age, length, and maturity from fisheries independent surveys and commercial data sources. 

# Results

The resulting visualizations for an example stock (Figure 1 and 2).

Some title for the left page:
...
Some title for the right page (something about samples):
...
- plan to include more climate related visualization such as centre of gravity and mean depth through time
- ...
