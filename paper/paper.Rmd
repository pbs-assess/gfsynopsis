...
Modern survey and fishery observation programs generate vast quantities of data. However, regulatory agencies often lack the capacity to translate that data into effective monitoring information. Here, we describe an approach of automated large-scale raw fisheries visualization and demonstrate it with nearly all data on 113 groundfish species in Pacific Canadian waters. Our implementation includes standardized visualizations of temporal trends and spatial distributions of commercial catches and survey indices, plus analyses of ages, lengths, maturity, and growth all on two pages per species. The approach facilitates discussions on stock-assessment and survey prioritization, increases transparency about data holdings, and makes the data available for regular review by interested parties. We encourage others to consider similar approaches for their own data.
...
However, we contend this is a case where "the whole is greater than the sum of its parts"---having such a variety of plot types in close proximity enables comparison and a rapid understanding of any stock (assessed or unassessed) and its data availability in a way that is not otherwise possible.
...
<!-- Paragraph 3: Specific ways in which this approach has proven useful. -->
Internally, the approach we describe here provides many benefits that we imagine would extend to any agency taking a similar approach (Table \@ref(tab:benefits)).
...
<!-- Paragraph X: This would be useful elsewhere. --> 
