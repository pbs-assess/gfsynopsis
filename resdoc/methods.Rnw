...
Extracting survey biological data from \texttt{GFBio} with \texttt{get-survey-samples.sql} & Filtered for \texttt{TRIP\_SUBTYPE\_CODE} \texttt{2, 3} (research trips)                                                                                                                       & To extract only research data                                                                                                              \\
                                                                                           ...
                                                                                           & Filtered for \texttt{SAMPLE\_SOURCE\_CODE} \texttt{1} (unsorted samples)                                                                                                                      & To extract only samples which were unsorted (some of which are discarded) to avoid sampling bias                                           \\
                                                                                           & Filtered for \texttt{USABILITY\_CODE} \texttt{0, 1, 2, 6} (usable samples)                                                                                                                       & To remove samples from incomplete surveys, when gear was damaged, or other sampling errors                                                 \\
                                                                                           & Filtered for \texttt{FE\_PARENT\_EVENT\_ID} \texttt{NULL} (to include data at only one level --- some data also recorded in database at higher or lower level with separate fishing event ID's) & To extract data at only one level --- some data are also recorded in the database at higher or lower levels with separate fishing event ID's \\
\bottomrule
\end{tabu}}
\end{table}

% Extracting commercial trawl catch per unit effort (kg/hr) from \texttt{GFFOS.GF\_D\_              & filtered for \texttt{LAT} between 47.8 and 55 and LON between -135 and -122                                                                                                          & To remove erroneous location records                                                                                                       \\
% \texttt{get-cpue.sql}                                               OFFICIAL\_CATCH}            & filtered for \texttt{YEAR(BEST\_DATE)} \textgreater2012                                                                                                                              & To extract data since the fishery footprint was initiated                                                                                  \\
...
% Extract commercial landings                                GFFOS.MERGED\_CATCH        & no filters                                                                                                                                                                  & To extract all landings and discards by fishery sector and gear                                                                            \\
% \texttt{get-landings.sql}                                                                      &                                                                                                                                                                             &                                                                                                                                            \\
