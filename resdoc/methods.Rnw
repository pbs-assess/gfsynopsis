...
plotted to visualise the available data for groundfish species. Commercial and
research catch, effort, and biological data are archived by the Groundfish Data
Unit (Fisheries and Oceans Canada, Science Branch, Pacific Region) and housed in
a number of relational databases. Historical commercial catch and effort data
from 1954 – 2006/2007 are housed in GFCatch, PacHarvTrawl, PacHarvHL, and
PacHarvSable depending on the fishery and time period. Modern (2006/2007 to
present) commercial catch data are housed in GFFOS, a groundfish-specific
“view” of the Fishery Operations System (FOS) database (Fisheries and Oceans
Canada, Fisheries and Aquaculture Management, Pacific Region).  Research survey
data and commercial biological data from the 1940s to present are housed in
GFBio, the Groundfish Biological Samples database.

Code was developed in this project to automate data extraction from these
databases in a consistent, reproducible manner. The data extraction relies on
SQL queries, developed with support from the Groundfish Data Unit, run within
the R functions to pull in data. The extracted data are formatted to feed
directly into other functions in the package to allow for a fully automated
procedure.

Some filtering is performed within the SQL queries to restrict the data extracted.
Commercial catch per unit effort (CPUE) index data are filtered to include only
records with valid start and end dates (Table X). Commercial trawl spatial CPUE
data are filtered to remove records with erroneous latitude and longitude values,
and to include only records from the groundfish trawl sector since 2012 when the
trawl footprint was implemented (Table X). Spatial longline CPUE data are similarly
filtered by latitude and longitude, gear type and for records from and including
2008 after the implementation of rockfish conservation areas (Table X).

Survey biomass index data were filtered for active records of a calculated
bootstrapped biomass column in the dataset (Table X). Biological data, including
length, weight, age, maturity and age, are extracted by different functions for
survey and commercial data, each filtered by the TRIP_SUBTYPE_CODE for either
survey or commercial samples. In addition, commercial samples were restricted
to unsorted samples to remove any bias from inclusion of sorted samples that
may skew the data, and survey samples were filtered for only total or random
samples to similarly avoid biases. The ageing precision data were filtered to
bring in only the primary and secondary readings, which are performed by separate
technicians.

Commercial catch data include all landings and discards by fishery sector and
gear type and were not filtered in any way.
...
\midrule
Extracting biological data from \texttt{GFBio} with \texttt{get-comm-samples.sql} and \texttt{get-survey-samples.sql} & Filtered \texttt{TRIP\_SUBTYPE\_CODE} \texttt{2, 3} for research trips or filtered out \texttt{2, 3} for commercial trips                    & To extract either only survey or commercial data                                                                                           \\
                                                                                                                      ...
                                                                                                                      & Filtered for \texttt{SPECIES\_CATEGORY\_CODE} \texttt{NULL, 1, 5, 6, 7} (unsorted catches)                                                   & To extract samples only from catches which were unsorted, including remains and fish heads from longline samples                           \\
                                                                                                                      & Filtered for \texttt{SAMPLE\_SOURCE\_CODE} \texttt{NULL, 1} (unsorted samples)                                                               & To extract only samples which were unsorted to avoid sampling bias                                                                         \\
\bottomrule
\end{tabu}}
\end{table}
...
SQL Query                                                                                                     & Filters                                                                                                                                   & Rationale                                                              \\
\midrule
Extracting commercial trawl spatial catch per unit effort (kg/hr) from \texttt{GFFOS.GF\_D\_OFFICIAL\_CATCH} with \texttt{get-cpue-spatial.sql}& Filtered for \texttt{LAT} between 47.8 and 55 and LON between -135 and -122                                & To remove erroneous location records                                   \\
