...
plotted to visualise the available data for groundfish species. Commercial
...
extracted. Commercial catch per unit effort (CPUE) index data were filtered
to include only records with valid start and end dates (Table X). Commercial
trawl spatial CPUE data were filtered to remove records with erroneous latitude
and longitude values, and to include only records from the groundfish trawl
sector since 2012 when the trawl footprint was implemented. Spatial longline
CPUE data were similarly filtered by latitude and longitude, gear type and for
records from and including 2008 after the implementation of rockfish
conservation areas.
...
bring in only the primary and secondary readings, which are performed by
separate technicians.

Commercial catch data include all landings and discards by fishery sector and gear type and were not filtered in any way.
